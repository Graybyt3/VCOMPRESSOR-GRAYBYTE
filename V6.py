...
class RainbowBar(tqdm):
    ...
    def format_bar(self):
        displayed = int(self.n) if self.total else 0
        bar_length = len(self.format_dict['bar'])
        filled = int(displayed / 100 * bar_length) if self.total else 0
        bar = ""
    ...
    def refresh(self, *args, **kwargs):
        self.desc = self.format_dict.get('desc', '')
        self.bar = self.format_bar()
...
def process_video(file):
    try:
        ...
        cmd = [
            ...
            "-i", str(file),
            "-c:v", "libx265", "-crf", "23", "-preset", "faster",
            "-c:a", "copy",
            "-threads", "4",
        ...
        if process.returncode == 0 and out_file.exists() and new_size > 0:
            ...
            terminal_line = (
                f"{Fore.GREEN}{Style.UNDERLINE}"
                f"⏱️ TIME TAKEN: {elapsed} | "
                f"{Fore.RED}OLD: {original_size:.2f} MB{Style.RESET_ALL}{Fore.WHITE} → {Style.RESET_ALL}"
                f"{Fore.GREEN}NEW: {new_size:.2f} MB{Style.RESET_ALL} | "
                f"💾 SAVED: {Fore.GREEN if space_saved >= 0 else Fore.RED}"
                f"{space_saved:+.2f} MB"
                f"{Style.RESET_ALL}\n"
            )

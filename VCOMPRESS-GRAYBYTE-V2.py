...
EXPLANATIONL = f"{Fore.GREEN}Multi-format H.265 Video Compressor | Resumable  | Logs → VCOM-CONFIG/{Style.RESET_ALL}\n"
PLAIN_EXPLANATION = "Multi-format H.265 Video Compressor | Resumable | Logs → VCOM-CONFIG/"
def strip_ansi(text):
...
log_only(f"CPU LIMIT: {CPU_LIMIT}% | INPUT: {input_dir}\n")
processed_files = load_processed()
# ==================== START VISUAL ENHANCEMENT CONFIG  ====================
# Set to None to disable specific enhancement
ENHANCE_BRIGHTNESS = 0.0      # +5% BLACK POINT LIFT
ENHANCE_CONTRAST   = 1.10      # +18% contrast
ENHANCE_SATURATION = 1.35      # +40% saturation
ENHANCE_SHARPNESS  = 0.00      # +4.5% sharpness
...
def get_new_filename(original_path):
    stem = original_path.stem.replace("-compressed", "").replace("~EncodeD", "")
    return output_dir / f"{stem}-enc{original_path.suffix.lower()}"
def process_video(file):
    try:
        ...
        if out_file.exists():
            msg = f"{Fore.MAGENTA}SKIPPING | FILE EXIST : {out_file.name}{Style.RESET_ALL}"
            log_and_print(msg)
        ...
        log_and_print(f"{Fore.CYAN}{'='*90}{Style.RESET_ALL}")
        log_and_print(f"\n{Fore.GREEN}PROCESSING |{Style.RESET_ALL} {Fore.CYAN}{file.name}{Style.RESET_ALL} | SIZE:{Fore.YELLOW}{original_size:.2f}MB{Style.RESET_ALL} | DUR:{Fore.GREEN}{duration_str}{Style.RESET_ALL}\n")
        ...
        with RainbowBar(total=100, bar_format="{l_bar}{bar}", ncols=80, leave=False) as bar:
            bar.set_description(f"{Fore.YELLOW}ELAPSED: {Style.RESET_ALL}{Fore.GREEN}0:00:00{Style.RESET_ALL} | REM: {Fore.YELLOW}{duration_str} {Style.RESET_ALL}")
        ...
        if process.returncode == 0 and out_file.exists():
            ...
            savings_msg = f"{Fore.YELLOW}TIME TOOK:{elapsed}|{Style.RESET_ALL}{Fore.RED} OLD-SIZE = {original_size:.2f} MB {Style.RESET_ALL}|{Fore.GREEN}NEW-SIZE = {new_size:.2f} MB {Style.RESET_ALL}|{Fore.GREEN}SPACE-SAVED = {space_saved:.2f} MB {Style.RESET_ALL}\n"
            log_and_print(success_msg)
            log_and_print(savings_msg)
            log_and_print(f"{Fore.YELLOW}{'='*90}{Style.RESET_ALL}")
            mark_processed(full_path)
            return True
        else:
        ...
        err = f"{Fore.RED}EXCEPTION | {file.name}: {Style.RESET_ALL}\n"
        log_and_print(err)
